...
public:
  ...
  quad_state()
  {
    ...
    sub2 = n.subscribe<geometry_msgs::PoseStamped>("/mocap_node/Quadrotor_2/pose", 1000, &quad_state::callback2, this);
  ...
  void callback2(const geometry_msgs::PoseStamped::ConstPtr& msg)
  {
    ...
    // Shift axes
    x = (msg->pose.position.x) - state.x_offset;
    y = -(msg->pose.position.y) - state.y_offset;
    z = msg->pose.position.z - state.z_offset;
